...
# ---------------------------------------------
# QAP Instance Data (Tai20b) – copied from your file
# ---------------------------------------------
MAX_ITERS = 100
SIZE = 20
...
def compute_cost(solution: Iterable[int]) -> int:
    ...
    for i in range(SIZE):
        for j in range(SIZE):
            total += distance[i][j] * flow[sol[i]][sol[j]]
    return int(total)


# ---------------------------------------------
# Generate all neighbors (swap moves)
# ---------------------------------------------
...
# ---------------------------------------------
# Worker function (hill climbing)
# ---------------------------------------------
def hill_climb_worker(seed: int) -> Tuple[int, List[int], int, int]:
    ...
    while iteration < MAX_ITERS:
        ...
        neighbor_costs = np.array([compute_cost(n) for n in neighbors])

        ranked = np.argsort(neighbor_costs)
        improved = False
        ...
        for idx in ranked:
            ...
            if candidate_cost < best_cost:
                ...
                current_cost = best_cost = candidate_cost
                best_solution = current_solution.copy()
                improved = True
                break
    ...
    return (seed, best_solution.tolist(), best_cost, iteration)


# ---------------------------------------------
# Main: parallel execution using Pool.map
# ---------------------------------------------
def main():
    ...
    seeds = rng.integers(0, 1_000_000, size=worker_count)
    print("Seeds:", seeds)

    start = time.time()

    with mp.Pool(processes=worker_count) as pool:
        results = pool.map(hill_climb_worker, seeds)

    elapsed = time.time() - start
    print(f"Parallel execution time: {elapsed:.4f} s\n")

    print("Results:")
    for seed, best_sol, best_cost, iters in results:
        print(f"seed={seed}  cost={best_cost}  iters={iters}")

    # find best
    best_run = min(results, key=lambda r: r[2])
    print("\nBest run:")
    print(best_run)
